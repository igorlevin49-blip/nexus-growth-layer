...
Deno.serve(async (req) => {
  ...
  try {
    // Verify authorization - must be superadmin
    ...
    if (roleError || !roleData) {
      console.error("User does not have superadmin role:", user.id);
      return new Response(
    ...
    console.log(`Superadmin ${user.id} authorized to send notifications`);
