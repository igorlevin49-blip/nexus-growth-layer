...
BEGIN
  ...
  -- Partner's payment date in the current month (if any)
  partner_current_payments AS (
    SELECT
      n.user_id,
      max(s.paid_at) as paid_at
    FROM network n
    JOIN subscriptions s ON s.user_id = n.user_id
      AND s.status = 'active'
  ...
  commission_status AS (
    SELECT
      ...
      CASE
        ...
        -- Root user (sponsor) was inactive in the partner's payment month
        WHEN (
          COALESCE((
            SELECT ma.is_activated
            FROM monthly_activations ma
            JOIN partner_current_payments pcp ON pcp.user_id = n.user_id
            WHERE ma.user_id = root_user_id
              AND ma.year = extract(year from pcp.paid_at)::int
              AND ma.month = extract(month from pcp.paid_at)::int
          ), false) = false
        ) AND NOT EXISTS (
          SELECT 1 FROM transactions t
          WHERE t.source_id IN (
