...
-- Drop the old 2-argument version first
DROP FUNCTION IF EXISTS public.get_referral_network_from_table(uuid, integer);

-- Now update the 3-argument version with security fixes
...
CREATE OR REPLACE FUNCTION public.get_referral_network_from_table(root_user_id uuid, max_level integer DEFAULT 10, p_structure_type integer DEFAULT 1)
 RETURNS TABLE(user_id uuid, partner_id text, level integer, full_name text, email text, phone text, avatar_url text, subscription_status text, subscription_expires_at timestamp with time zone, monthly_activation_met boolean, referral_code text, created_at timestamp with time zone, direct_referrals bigint, total_team bigint, monthly_volume numeric, parent_partner_id text, parent_user_id uuid, has_commission_received boolean, no_commission_reason text, commission_status text, commission_frozen_until timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Security: Only allow if user is viewing their own network or is admin
  ...
  -- If not admin, must be viewing own network
  IF NOT is_admin_user AND calling_user_id != root_user_id THEN
    RAISE EXCEPTION 'Access denied: can only view your own network';
  END IF;
  ...
  network_with_profiles AS (
    SELECT
      ...
      p.full_name,
      -- SECURITY: Mask email and phone for non-admin users
      CASE 
        WHEN is_admin_user OR p.id = calling_user_id THEN p.email
        ELSE NULL
      END as email,
      CASE 
        WHEN is_admin_user OR p.id = calling_user_id THEN p.phone
        ELSE NULL
      END as phone,
      p.avatar_url,
...
COMMENT ON FUNCTION public.get_referral_network_from_table(uuid, integer, integer) IS 'Returns network tree with privacy protection: email/phone visible only to admins or the user themselves. Access restricted to own network or admins.';
