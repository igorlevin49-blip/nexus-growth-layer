-- Fix backfill function with correct JSONB syntax
...
DECLARE
  ...
  v_l1_created INTEGER := 0;
  v_l2_created INTEGER := 0;
  v_l3_created INTEGER := 0;
  v_l4_created INTEGER := 0;
  v_l5_created INTEGER := 0;
  v_l1_amount NUMERIC := 0;
  v_l2_amount NUMERIC := 0;
  v_l3_amount NUMERIC := 0;
  v_l4_amount NUMERIC := 0;
  v_l5_amount NUMERIC := 0;
...
BEGIN
        ...
        IF NOT p_dry_run THEN
          ...
          IF FOUND THEN
            ...
            CASE v_current_level
              WHEN 1 THEN v_l1_created := v_l1_created + 1; v_l1_amount := v_l1_amount + v_commission_amount;
              WHEN 2 THEN v_l2_created := v_l2_created + 1; v_l2_amount := v_l2_amount + v_commission_amount;
              WHEN 3 THEN v_l3_created := v_l3_created + 1; v_l3_amount := v_l3_amount + v_commission_amount;
              WHEN 4 THEN v_l4_created := v_l4_created + 1; v_l4_amount := v_l4_amount + v_commission_amount;
              WHEN 5 THEN v_l5_created := v_l5_created + 1; v_l5_amount := v_l5_amount + v_commission_amount;
            END CASE;
          END IF;
        ELSE
          -- Dry run - just count
  ...
  IF NOT p_dry_run AND v_total_created > 0 THEN
    INSERT INTO admin_actions (
      ...
      jsonb_build_object(
        ...
        'L1', jsonb_build_object('created', v_l1_created, 'amount', v_l1_amount),
        'L2', jsonb_build_object('created', v_l2_created, 'amount', v_l2_amount),
        'L3', jsonb_build_object('created', v_l3_created, 'amount', v_l3_amount),
        'L4', jsonb_build_object('created', v_l4_created, 'amount', v_l4_amount),
        'L5', jsonb_build_object('created', v_l5_created, 'amount', v_l5_amount)
      ),
  ...
  RETURN json_build_object(
    ...
    'levels_stats', jsonb_build_object(
      'L1', jsonb_build_object('created', v_l1_created, 'amount', v_l1_amount),
      'L2', jsonb_build_object('created', v_l2_created, 'amount', v_l2_amount),
      'L3', jsonb_build_object('created', v_l3_created, 'amount', v_l3_amount),
      'L4', jsonb_build_object('created', v_l4_created, 'amount', v_l4_amount),
