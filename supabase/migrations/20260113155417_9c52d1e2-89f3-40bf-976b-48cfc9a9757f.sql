...
-- Исправляем функцию - роль хранится в user_roles, не в profiles
...
DECLARE
  ...
  v_sponsor_name TEXT;
BEGIN
  -- Проверка прав администратора через user_roles
  ...
  IF NOT p_dry_run AND v_commissions_created > 0 THEN
    ...
    VALUES (
      p_admin_id, 'backfill_multilevel_commissions',
      jsonb_build_object(
