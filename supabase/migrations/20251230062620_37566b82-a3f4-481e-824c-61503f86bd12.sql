-- Fix function to use correct column names from profiles table
...
BEGIN
  ...
  network_with_profiles AS (
    SELECT 
      ...
      n.level,
      n.parent_user_id,
      p.id::text as partner_id,  -- Use id as partner_id
      p.full_name,
      p.email,
      p.phone,
      p.referral_code,
      p.avatar_url,
      p.created_at,
      parent_p.id::text as parent_partner_id  -- Use id as parent_partner_id
    FROM network n
