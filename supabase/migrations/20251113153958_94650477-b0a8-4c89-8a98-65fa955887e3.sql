-- Fix user_metadata column reference error in auto-bind functions
-- The column 'user_metadata' does not exist in auth.users table
-- We should only use raw_user_meta_data and raw_app_meta_data

-- Fix auto_bind_sponsor_from_metadata function
...
BEGIN
  ...
  IF NEW.sponsor_id IS NULL THEN
    -- Try to get inviter code from raw_user_meta_data only (fixed from previous version)
    SELECT COALESCE(
...
-- Fix admin_backfill_sponsor_from_metadata function
