...
-- Fix: Улучшенная логика определения причины отсутствия комиссии
CREATE OR REPLACE FUNCTION public.get_referral_network_from_table(root_user_id uuid, max_level integer DEFAULT 10, p_structure_type integer DEFAULT 1)
 RETURNS TABLE(user_id uuid, partner_id uuid, parent_partner_id uuid, level integer, full_name text, email text, referral_code text, subscription_status text, monthly_activation_met boolean, created_at timestamp with time zone, avatar_url text, direct_referrals integer, total_team integer, monthly_volume bigint, has_commission_received boolean, no_commission_reason text)
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  commission_status AS (
    SELECT 
      ...
      CASE
        ...
        WHEN (SELECT subscription_status FROM profiles WHERE id = n.user_id) != 'active' THEN 'not_activated'
        ...
        WHEN NOT EXISTS (
          ...
          SELECT COALESCE(
            NOT monthly_activation_completed,
