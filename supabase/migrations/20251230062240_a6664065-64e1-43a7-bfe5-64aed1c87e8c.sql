-- Fix get_referral_network_from_table to correctly check actual commission transactions
-- and handle marketing_free_access users properly
  ...
  SELECT 
    ...
    (
      WITH RECURSIVE team AS (
        SELECT r.referred_id FROM referrals r WHERE r.referrer_id = nwa.user_id
        UNION ALL
        SELECT r.referred_id FROM referrals r INNER JOIN team t ON r.referrer_id = t.referred_id
      )
      SELECT COUNT(*) FROM team
    )::bigint as total_team,
    -- Monthly volume placeholder
    0::numeric as monthly_volume,
    -- FIXED: Check actual commission transaction exists for this partner
    EXISTS (
    ...
    -- FIXED: Determine reason for no commission with marketing_free_access check
    CASE
