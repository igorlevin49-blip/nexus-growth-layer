...
-- Функция для backfill пропущенных L2-L5 комиссий
CREATE OR REPLACE FUNCTION public.backfill_missing_multilevel_commissions(
  p_admin_id UUID,
  p_dry_run BOOLEAN DEFAULT TRUE,
  p_target_user_id UUID DEFAULT NULL  -- Опционально: только для конкретного спонсора
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Получаем все активные подписки с оплатой
  FOR v_sub IN 
    SELECT 
    ...
    WHERE s.status = 'active'
      ...
      -- Если указан target_user_id, проверяем только подписки в его структуре
      AND (p_target_user_id IS NULL OR EXISTS (
        -- Проверяем что подписчик в цепочке под target_user_id
  ...
  LOOP
    ...
    -- Проходим по уровням 2-5 (L1 уже обрабатывается существующим backfill)
    v_current_id := v_sub.direct_sponsor_id;
    
    FOR v_level IN 2..5 LOOP
      -- Получаем спонсора на текущем уровне
      SELECT sponsor_id INTO v_sponsor_id
      FROM profiles 
      WHERE id = v_current_id;
      
      -- Если спонсора нет - выходим из цикла
      IF v_sponsor_id IS NULL THEN
        EXIT;
      END IF;
      
      -- Проверяем только если это target_user или target не указан
      IF p_target_user_id IS NULL OR v_sponsor_id = p_target_user_id THEN
        ...
        -- Проверяем что комиссия ещё не начислена
        ...
        IF v_existing_commission IS NULL THEN
          -- Определяем ставку комиссии и требуемых рефералов
          CASE v_level
          ...
          -- Проверяем активность подписки спонсора на момент оплаты подписчика
          SELECT EXISTS (
            SELECT 1 FROM subscriptions
            WHERE user_id = v_sponsor_id
              AND status = 'active'
              AND paid_at <= v_sub.paid_at
          ) INTO v_sponsor_active;
          
          -- Считаем активных рефералов первой линии спонсора на момент оплаты
...
-- Комментарий
COMMENT ON FUNCTION public.backfill_missing_multilevel_commissions IS 
'Backfill пропущенных L2-L5 комиссий. Проверяет условия разблокировки уровней и активность спонсоров.';
