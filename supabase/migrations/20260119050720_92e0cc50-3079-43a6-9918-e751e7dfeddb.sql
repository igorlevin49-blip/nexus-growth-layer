-- Fix the backfill function to use unique source_ref
...
BEGIN
  ...
  LOOP
    ...
    v_source_ref := 'subscription:' || v_record.subscription_id::text || ':L1:' || v_record.sponsor_id::text || ':backfill_' || to_char(now(), 'YYYYMMDDHH24MISS');
