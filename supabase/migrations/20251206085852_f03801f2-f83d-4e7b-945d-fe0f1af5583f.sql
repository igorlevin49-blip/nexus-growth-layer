-- Update get_referral_network_from_table to include subscriptions in monthly_volume calculation
...
BEGIN
  ...
  team_counts AS (
    -- Calculate total team size for each member using recursive CTE
    SELECT 
      n.member_id,
      (
  ...
  SELECT 
    ...
    -- Monthly volume: orders + subscriptions for current month
    (
      COALESCE((
        ...
        WHERE s.user_id = p.id
          AND s.status = 'active'
          AND DATE_TRUNC('month', s.paid_at) = DATE_TRUNC('month', NOW())
      ), 0)
    ) as monthly_volume,
