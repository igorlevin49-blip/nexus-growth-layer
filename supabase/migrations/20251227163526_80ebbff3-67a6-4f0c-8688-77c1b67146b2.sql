-- Fix: Replace incorrect 'completed' status with 'paid' in get_referral_network_from_table
...
BEGIN
  ...
  team_sizes AS (
    SELECT 
      n.user_id,
      (SELECT COUNT(*) FROM profiles WHERE sponsor_id = n.user_id AND deleted_at IS NULL AND subscription_status = 'active')::INT as direct,
      (
