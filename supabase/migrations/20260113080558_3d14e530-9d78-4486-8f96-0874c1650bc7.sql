-- Fix get_referral_network_from_table to restore is_marketing_free_access check
-- This was lost in migration 20260112094552
...
BEGIN
  ...
  WITH RECURSIVE network AS (
    ...
    SELECT 
      ...
      1 AS lvl,
      n.user_id AS parent
    FROM referral_network n
    WHERE n.user_id = root_user_id
      AND n.structure_type = p_structure_type
    ...
    SELECT 
      ...
      n.user_id AS parent
    FROM referral_network n
    INNER JOIN network net ON n.user_id = net.id
    WHERE n.structure_type = p_structure_type
  ...
  SELECT 
    ...
    -- Determine reason for no commission - NOW WITH is_marketing_free_access check
    CASE 
