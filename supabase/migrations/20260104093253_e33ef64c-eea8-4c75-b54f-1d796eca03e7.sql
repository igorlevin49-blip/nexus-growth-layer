-- Fix backfill_missing_s1_commissions: target_id must be UUID, not string
-- Using gen_random_uuid() for batch operations log
...
BEGIN
  ...
  -- Log admin action (use gen_random_uuid() for batch operation target_id)
