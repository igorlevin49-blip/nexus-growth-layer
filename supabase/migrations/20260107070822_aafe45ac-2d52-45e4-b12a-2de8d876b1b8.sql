...
-- Fix ambiguous column reference in get_referral_network_from_table
-- The issue is that "user_id" exists in both profiles (as id) and monthly_activations
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(mact.is_activated, false),
    COALESCE(prof.direct_referrals_count, 0)
  INTO root_subscription_status, root_activation_met, root_direct_referrals
  FROM profiles prof
  LEFT JOIN monthly_activations mact ON mact.user_id = prof.id 
    AND mact.year = EXTRACT(YEAR FROM CURRENT_DATE)::integer
    AND mact.month = EXTRACT(MONTH FROM CURRENT_DATE)::integer
  ...
  current_activations AS (
    SELECT 
      mact.user_id AS uid,
      mact.is_activated
    FROM monthly_activations mact
    WHERE mact.year = EXTRACT(YEAR FROM CURRENT_DATE)::integer
      AND mact.month = EXTRACT(MONTH FROM CURRENT_DATE)::integer
  ),
