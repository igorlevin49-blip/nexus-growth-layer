...
-- Fix function to use referral_code instead of partner_id
...
BEGIN
  ...
  marketing_free_access AS (
    ...
    WHERE s.status = 'active' AND s.is_marketing_free_access = true
  ),
  commission_info AS (
    SELECT (t.payload->>'from_user_id')::uuid as source_user_id, TRUE as has_commission
    FROM transactions t
    WHERE t.user_id = root_user_id AND t.type = 'commission' AND t.level = 1
      AND t.created_at >= date_trunc('month', CURRENT_DATE)
      AND t.created_at < date_trunc('month', CURRENT_DATE) + interval '1 month'
    GROUP BY (t.payload->>'from_user_id')::uuid
  ),
  direct_counts AS (
    SELECT r.referrer_id, COUNT(*) as direct_count
    FROM referrals r WHERE r.structure_type = p_structure_type
    GROUP BY r.referrer_id
  ),
  team_sizes AS (
    SELECT n.user_id,
      (WITH RECURSIVE sub_network AS (
        SELECT r2.referred_user_id FROM referrals r2
        WHERE r2.referrer_id = n.user_id AND r2.structure_type = p_structure_type
        UNION ALL
        SELECT r3.referred_user_id FROM referrals r3
        INNER JOIN sub_network sn ON r3.referrer_id = sn.referred_user_id
        WHERE r3.structure_type = p_structure_type
      ) SELECT COUNT(*) FROM sub_network) as team_count
    FROM network n
  ),
  monthly_volumes AS (
    SELECT n.user_id, COALESCE(SUM(o.total_usd), 0) as volume
    FROM network n
    LEFT JOIN orders o ON o.user_id = n.user_id AND o.status = 'paid'
