-- Fix get_referral_network_from_table function to restore p_structure_type parameter
-- The parameter was accidentally removed in a previous migration
...
BEGIN
  ...
  SELECT
    ...
    -- Commission received (only relevant for S1)
    CASE WHEN p_structure_type = 1 THEN
      EXISTS (
        SELECT 1 FROM transactions t
        WHERE t.source_id = nwa.user_id
