...
-- Fix no_payment_this_month logic for S1 structure
-- In S1 (subscription structure), partner's monthly activation doesn't affect sponsor's commission
-- Only partner's subscription status matters for S1
  ...
  network_with_profiles AS (
    SELECT
      ...
      CASE
        ...
        -- FIX: no_payment_this_month only applies to S2 (product structure)
        -- In S1 (subscription structure), partner's monthly activation doesn't affect sponsor's commission
        WHEN p_structure_type = 2 AND NOT COALESCE(ma.is_activated, p.monthly_activation_completed, false) THEN 'no_payment_this_month'
