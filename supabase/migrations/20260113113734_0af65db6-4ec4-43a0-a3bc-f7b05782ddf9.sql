...
CREATE OR REPLACE FUNCTION reconcile_s1_commissions(
  p_sponsor_id uuid, p_depth integer DEFAULT 5,
  p_from_date timestamptz DEFAULT NULL, p_to_date timestamptz DEFAULT NULL
)
RETURNS TABLE (
  subscriber_id uuid, subscriber_name text, subscriber_email text,
  network_level integer, subscription_id uuid, subscription_status text,
  paid_at timestamptz, amount_kzt numeric, is_marketing_free_access boolean,
  expected_commission_kzt numeric, actual_transaction_id uuid,
  actual_status text, actual_amount_kzt numeric, missing_reason text
) AS $$
DECLARE v_sponsor_sub_started timestamptz;
BEGIN
  SELECT started_at INTO v_sponsor_sub_started FROM subscriptions 
  WHERE user_id = p_sponsor_id AND status = 'active' ORDER BY started_at ASC LIMIT 1;

  RETURN QUERY
  WITH RECURSIVE sponsor_network AS (
    SELECT r.referred_user_id AS user_id, 1 AS lvl FROM referrals r 
    WHERE r.referrer_id = p_sponsor_id AND r.structure_type = 1
    UNION ALL
    SELECT r.referred_user_id, sn.lvl + 1 FROM sponsor_network sn
    JOIN referrals r ON r.referrer_id = sn.user_id AND r.structure_type = 1 WHERE sn.lvl < p_depth
  ),
  commission_rules AS (
    SELECT mcr.level AS rule_level, mcr.percent AS rule_percent FROM mlm_commission_rules mcr
  ...
  network_subscriptions AS (
    SELECT DISTINCT ON (sn.user_id)
      ...
      COALESCE(s.is_marketing_free_access, false) AS is_free, p.full_name, p.email
  ...
  existing_commissions AS (
    SELECT t.id AS tx_id, t.source_id, t.status::text AS tx_status,
           t.amount_cents::numeric AS tx_amount, t.level AS tx_level
    FROM transactions t WHERE t.user_id = p_sponsor_id AND t.type = 'commission'
      AND t.structure_type = 'primary' AND t.source_id IS NOT NULL
  )
  SELECT ns.user_id, ns.full_name, ns.email, ns.lvl, ns.sub_id, ns.sub_status,
    ns.sub_paid_at, ns.sub_amount_kzt, ns.is_free,
    CASE WHEN ns.is_free THEN 0::numeric ELSE COALESCE(ns.sub_amount_kzt * cr.rule_percent / 100, 0)::numeric END,
